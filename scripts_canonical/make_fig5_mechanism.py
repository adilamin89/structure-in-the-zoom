...
plt.subplots_adjust(wspace=0.32, left=0.06, right=0.985, top=0.90,
                    bottom=0.16)
...
ax.text(-0.42, 0.248, "GT3 observed $+0.237$", color="#c23b3b", fontsize=8,
        ha="left", va="bottom")
ax.set_xticks(xs)
ax.set_xticklabels([b[0] for b in bars], fontsize=8)
ax.set_ylabel(r"model $\delta$")
ax.set_ylim(0, 0.56)
ax.set_title("(b) two knobs bracket the data", fontsize=10)
ax.annotate("overshoot\n(known defect)", xy=(2, run11b["boot_mean"]),
            xytext=(1.25, 0.42), fontsize=7.5,
...
ax.errorbar([2 + w + 0.16], [dig["delta_digit_mean"]],
            ...
            label=r"$\delta_{\mathrm{digit}}$ (invariant)")
ax.axhline(0, color="k", lw=0.6)
ax.set_xticks(np.arange(3))
ax.set_xticklabels(layers)
ax.set_ylabel(r"$\delta_{\mathrm{rot}}$ (5 seeds)")
ax.legend(fontsize=7, frameon=False, loc="upper left")
ax.set_title("(c) architectural null vs carried structure", fontsize=10)

# inset: tuned fraction by depth + stimulus baseline
ins = ax.inset_axes([0.585, 0.42, 0.39, 0.34])
...
ins.text(0.02, run14["raw_pixels"]["tuned_fraction"] * 1.25, "pixels",
         fontsize=6, color="#c23b3b")
ins.set_yscale("log")
ins.set_xticks([0, 1, 2])
ins.set_xticklabels(["c1", "c2", "c3"], fontsize=6)
