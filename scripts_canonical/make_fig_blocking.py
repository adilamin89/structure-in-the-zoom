...
block(axA, 8.55, [0, np.pi, 0, np.pi], "sorted by orientation\n(antipodal preferences)", "cancels, $\\rho_1 \\approx 0$", "adds, $\\rho_2 \\approx 0.2$", BLUE)
...
ax.legend(frameon=False, loc="upper left", handlelength=1.3); ax.text(0.03, 0.03, "dashed: Eq. 3 at\nthe measured $\\rho$", transform=ax.transAxes, ha="left", va="bottom", fontsize=5.6, color="0.35")
...
ax.plot([32], [1.0], "o", color=INK, ms=4.5, zorder=5); ax.text(32, 0.84, "mouse, measured", fontsize=5.6, ha="center", va="top", color=INK)
ax.set_xscale("log", base=2); ax.set_yscale("log"); ax.set_xticks([1, 4, 16, 64, 256]); ax.set_xticklabels(["1", "4", "16", "64", "256"])
ax.set_ylim(0.6, 60); ax.set_xlabel("units pooled $K$"); ax.set_ylabel("$B(K)$", labelpad=1)
