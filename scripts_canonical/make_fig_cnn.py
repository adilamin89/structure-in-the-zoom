...
axA = fig.add_axes([0.005, 0.03, 0.40, 0.86]); axB = fig.add_axes([0.525, 0.17, 0.235, 0.70]); axC = fig.add_axes([0.84, 0.17, 0.15, 0.70])
...
ax = axA; ax.set_xlim(0, 10.4); ax.set_ylim(0, 10.8); ax.axis("off")
def box(x, y, w, h, text, fc="white", ec=GRAY, fs=6.8, col=INK):
...
for y, lab in zip(ys, labels):
    ...
    for i, name in enumerate(["conv1", "conv2", "conv3"]):
        box(3.75 + i * 1.35, y, 1.15, 1.0, name, fc=C_PRE, ec=C_PRE, col="white", fs=6.6)
    ax.annotate("", xy=(7.95, 5.35), xytext=(7.65, y + 0.5), arrowprops=dict(arrowstyle="-|>", color=GRAY, lw=0.6, mutation_scale=6))
ax.text(5.6, 10.75, "one shared trunk,\nweights tied across the four copies", fontsize=6.8, ha="center", va="top", color=INK, linespacing=1.0)
box(7.95, 4.45, 2.05, 1.8, "mean over\nthe orbit", fc=C_INV, ec=C_INV, col="white", fs=6.4)
ax.text(9.0, 4.05, "invariant by\nconstruction", fontsize=6.6, ha="center", va="top", color=C_INV, linespacing=1.0)
ax.text(7.0, 0.85, "read here: pre-pool features\n(equivariant)", fontsize=6.8, ha="center", va="top", color=C_PRE, linespacing=1.0)
ax.text(9.0, 6.6, "read here:\ninvariant", fontsize=6.8, ha="center", va="bottom", color=C_INV, linespacing=1.0)
ax.text(0.3, 1.95, "plain CNN: one copy,\nno pooling", fontsize=6.8, ha="left", va="top", color=GRAY, linespacing=1.0)
# ---- (B) delta_rot by layer ----
ax = axB; layers = ["conv1", "conv2", "conv3"]
groups = [("plain", ["plain/conv1", "plain/conv2", "plain/conv3"], C_PLAIN),
          ("pre-pool", ["equivariant/conv1_equi", "equivariant/conv2_equi", "equivariant/conv3_equi"], C_PRE),
...
ax.errorbar([2 + w + 0.16], [dig["delta_digit_mean"]], yerr=[dig["delta_digit_sd"]], fmt="D", ms=5, color=RED, label=r"$\delta_{\mathrm{digit}}$, invariant")
ax.axhline(0, color="k", lw=0.6); ax.set_xticks(np.arange(3)); ax.set_xticklabels(layers)
ax.set_ylabel(r"$\delta_{\mathrm{rot}}$ (5 seeds)"); ax.set_ylim(-0.035, 0.205)
ax.legend(fontsize=6.2, frameon=False, loc="upper left", ncol=2, handlelength=1.0, columnspacing=0.7, borderaxespad=0.2)
ax.set_title("B  the invariant zero", loc="left", fontweight="bold")
