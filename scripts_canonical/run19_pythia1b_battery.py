...
Out: feedback_runs/run19_pythia1b_battery.json
"""
import csv
import json
...
def get_hidden_states(model, tokenizer, prompts, device="mps", max_len=128):
    all_states = []
    model.eval()
    with torch.no_grad():
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained("EleutherAI/pythia-1b-deduped", torch_dtype=torch.float16).to("mps")
    ...
    out = {"model": "EleutherAI/pythia-1b-deduped", "n_layers": n_layers, "axes": {}}
    ...
    json.dump(out, open(HERE / "run19_pythia1b_battery.json", "w"),
              indent=1)
